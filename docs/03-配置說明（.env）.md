...
- `HEALTH_FIXED_INTERVAL_SEC`：固定心跳秒數（>0 啟用），例如 `600` 代表每 10 分鐘一則
- `TG_INCLUDE_SYSTEM_METRICS=1`：健康訊息顯示 CPU(load1)、RSS、磁碟空間
...
- `TG_ACTION_REFRESH_MIN_INTERVAL_SEC`：按鈕刷新最小間隔秒數（預設 15）
- `TG_ACTION_COMMAND_RATE_LIMIT_PER_MIN`：每位管理員每分鐘最多文字指令數（預設 8）
- `TG_ACTION_TIMEOUT_SEC`：按鈕命令逾時秒數（預設 3.0）
- `TG_ACTION_COMMAND_TIMEOUT_SEC`：文字指令逾時秒數（預設 10.0）
- `TG_ACTION_COMMAND_ALLOWLIST`：啟用的文字指令（預設 `help,db_stats,top_symbols,symbol`）
- `TG_ACTION_COMMAND_MAX_LOOKBACK_DAYS`：文字查詢可回看天數（預設 30）
...
TG_ACTION_COMMAND_TIMEOUT_SEC=12.0
TG_ACTION_COMMAND_ALLOWLIST=help,db_stats,top_symbols,symbol
TG_ACTION_COMMAND_MAX_LOOKBACK_DAYS=90
```

## 盤前 / 盤中 / 盤後建議

- 要固定心跳（建議值班用）：`HEALTH_FIXED_INTERVAL_SEC=600`
