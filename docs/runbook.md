...
- `ingest/min`：每分鐘流入量（push + poll accepted）
- `persist/min`：每分鐘落盤量
- `write_eff`：寫入效率（`persist/min` 除以 `ingest/min`）
- `stale_symbols`：超過 stale 門檻的 symbol 數
- `stale_bucket(...)`：stale 分桶
- `top5_stale`：最慢 5 個 symbol
- `close_snapshot_ok`：是否排空 queue
- `db_growth_today`：今日 DB 增量
...
## 4.3 檢查 webhook / long polling 衝突
